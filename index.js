...
console.log(createMenuItem('crepe', 8, 'Lunch'));
...
*/
const newReview = {name: 'nate', rating: 4, feedback: 'its ok',};
...
function getReviewByIndex(testArray, index) {
  return `${testArray[index].name} gave the restaurant a ${testArray[index].rating} star review, and their feedback was: ${testArray[index].feedback}`;
}
...
function getLastReview(testArray) {
  return `${testArray[testArray.length-1].name} gave the restaurant a ${testArray[testArray.length-1].rating} star review, and their feedback was: ${testArray[testArray.length-1].feedback}`
} 
